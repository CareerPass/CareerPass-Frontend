  ...
  return (
    <div className="absolute font-['Noto_Sans_KR:Medium',_sans-serif] font-medium h-[36px] leading-[normal] left-[690px] text-[27px] text-black text-nowrap top-[56px] w-[597px] whitespace-pre" data-name="목록버튼">
      <button 
        ...
        className="absolute bottom-[11.11%] left-[-15px] right-[80%] top-0 hover:text-blue-600 transition-colors cursor-pointer"
      >
        취업 로드맵
      </button>
      <button 
        onClick={() => onPageChange('resume')}
        className="absolute bottom-[11.11%] left-[32%] right-[42%] top-0 hover:text-blue-600 transition-colors cursor-pointer"
      >
        자기소개서 AI
      </button>
      <button 
        onClick={() => onPageChange('interview')}
        className="absolute bottom-[11.11%] left-[73%] right-0 top-0 hover:text-blue-600 transition-colors cursor-pointer"
      >
        AI 모의면접
      </button>
    </div>
...
function Component8({ onLoginClick }: { onLoginClick: () => void }) {
  return (
    <div 
      ...
      className="absolute left-[1331px] size-[62px] top-[46px] hover:scale-105 transition-transform cursor-pointer" 
      data-name="프로필"
    >
